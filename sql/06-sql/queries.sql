...
SELECT SUM(price) FROM products WHERE in_stock = 1;
SELECT AVG(price) FROM products;
SELECT MAX(price), MIN(price) FROM products;
SELECT COUNT(DISTINCT product_category) FROM products;
SELECT product_category, COUNT(*) FROM products GROUP BY product_category;
SELECT product_category, SUM(price) FROM products GROUP BY product_category;
SELECT product_category, AVG(price) FROM products GROUP BY product_category HAVING AVG(price) > 1000;
SELECT product_category FROM products GROUP BY product_category HAVING COUNT(*) > 1;
SELECT product_category, SUM(price) FROM products WHERE in_stock = 1 GROUP BY product_category HAVING SUM(price) > 200;
